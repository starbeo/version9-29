...
header("Content-Type: application/json; charset=UTF-8");
	include '../../../../../config/dbConnect.php';
        if((isset($_POST['Codebarre']) and !empty($_POST['Codebarre'])) 
           ...
           and (isset($_POST['datereportation']) and !empty($_POST['datereportation'])) 
		...
		$id_type="11";
		$datereportation=$_POST['datereportation'];
        $idemplacement="9";
		...
		$message.=",ID:";
		$message.=(int)$id_user;
		$dateDE = strtr($_POST['datereportation'], '/', '-');
		$newDate = date("Y-m-d",strtotime($dateDE));
...

{
     $sql="INSERT INTO `tblstatus`(code_barre,type,date_created,emplacement_id,date_reporte,id_utilisateur,	id_entreprise) 
       VALUES ('$Codebarre','". (int)$id_type . "',Now(),'". (int)$idemplacement . "','$newDate','". (int)$id_user . "',0)";
