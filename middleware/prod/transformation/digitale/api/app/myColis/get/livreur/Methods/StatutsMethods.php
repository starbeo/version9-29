...
public function GetStatusColisByCodeEnvoi($id){
    ...
    $query = "SELECT  `tblstatus`.`code_barre` as 'code_envoi', `tblstatuscolis`.`name` as 'statut',
             `tbllocations`.`name` as 'emplacement', tblstatuscolis.`color`,
             DATE_FORMAT(date_reporte,'%d-%m-%Y') as date_reporte,
             DATE_FORMAT(tblstatus.date_created,'%d-%m-%Y') as date_created
