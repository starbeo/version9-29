...
// query products
$stmt = $BonLivraisonMethods->GetAllBonLivraisonItemsByExpediteurPaging($from_record_num, $records_per_page,
...
if($num>0){
    ...
    $total_rows=$BonLivraisonMethods->nombrebonlivraisonItems(isset($_GET['client_id']) ? $_GET['client_id'] : die(),
    isset($_GET['bonlivrasion_id']) ? $_GET['bonlivrasion_id'] : die());
    $page_url="{$home_url_client}BonLivraison/getBonLivraisonItems.php?client_id=".(int)$expediteur."&bonlivrasion_id=".(int)$bonlivrasion_id."&tocken=$tocken&";
