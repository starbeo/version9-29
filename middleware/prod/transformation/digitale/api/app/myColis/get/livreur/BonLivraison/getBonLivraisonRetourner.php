...
// query products
$stmt = $BonLivraisonMethods->GetAllBonLivraisonRetournerPaging($from_record_num, $records_per_page,
...
if($num>0){
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        $bonlivraison_item=array(
            ...
            "typeBonLivraison" => $BonLivraisonMethods->type_bonlivraison($type)
        );
        array_push($bonlivraison_arr["records"], $bonlivraison_item);
    }
    // include paging
    $livreur_id=$_GET['livreur_id'];
    $total_rows=$BonLivraisonMethods->nombrebonlivraisonretourner(isset($_GET['livreur_id']) ? $_GET['livreur_id'] : die());
    $page_url="{$home_url_livreur}BonLivraison/getBonLivraisonRetourner.php?livreur_id=".(int)$livreur_id."&tocken=$tocken&";
