    ...
    <div class="content">
        <div class="row no-margin">
            ...
            <p class="text-center mbot0">
                <img src="<?= site_url('assets/images/defaults/menus/bons-de-livraison.png') ?>" class="img-detail" />
            </p>
            <h2 class="no-margin text-center bold"><?= $bon_livraison->nom ?></h2>
        </div>
            ...
            if ($total > 0) {
                foreach ($colis as $coli) {
                    ...
                    <div class="col-md-offset-4 col-md-4 col-sm-offset-3 col-sm-6 col-xs-12 bloc-colis">
                        <p class="text-center mbot5 bold"><?= format_status_colis($coli['status_reel'], '', 'fs20') ?></p>
                        <div class="row">
                        ...
                        <?php } ?>
                        <?php if ($bon_livraison->type == 1 && is_numeric($coli['status_reel']) && $coli['status_reel'] != 2 && $coli['status_reel'] != 3 && $coli['status_reel'] != 13) { ?>
                        <p class="mbot5 text-center">
                            <?= icon_btn('livreur/colis/detail/' . $coli['colis_id'], 'eye', 'btn-info btn-bloc') ?>
                            <?php if (is_numeric($coli['status_reel']) && $coli['status_reel'] != 2 && $coli['status_reel'] != 3 && $coli['status_reel'] != 13) { ?>
                                ...
                                <?= icon_btn('livreur/status/add/' . $coli['colis_id'], 'plus', 'btn-info btn-bloc') ?>
                            <?php } ?>
                        </p>
                        <?php } ?>
                    </div>
...
<script src="<?= site_url('assets/js/livreur/bons-livraison/detail.js?v=' . version_sources()); ?>"></script>
</body>
</html>
