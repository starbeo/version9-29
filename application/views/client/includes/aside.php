<aside id="menu" class="animated fadeIn">
    <ul class="nav metis-menu" id="side-menu">
        <li class="dashboard_user">
            <?= _l('welcome_top', ''); ?> <i class="fa fa-power-off top-left-logout pull-right" data-toggle="tooltip" data-title="<?= _l('nav_logout'); ?>" data-placement="left" onclick="window.location.href = '<?= site_url('authentication/logout_client'); ?>'"></i>
        </li>
        <?php
        $menu_active = get_option('aside_menu_active_client');
        $menu_active = json_decode($menu_active);
        foreach ($menu_active->aside_menu_active_client as $item) {
