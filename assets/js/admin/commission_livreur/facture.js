$(document).ready(function () {
    ...
    $('body').on('change', 'select[name="expediteurs"], select[name="types"]', function () {
        ...
        // console.log(input[name="id_expediteur"])
        $('input[name="type"]').val('');
        $('button[id="submit"]').attr('disabled', false);

        if (expediteur !== '' && type !== '') {
            initDataTable_1('.table-colis-facture', admin_url + 'commission_livreur/init_colis_facture/' + expediteur + '/' + type, 'colis-facture', [6, 7], [6, 7]);
    ...
    if (typeof (isedit) !== 'undefined') {
        ...
        console.log(type);
        initDataTable_1('.table-colis-facture', admin_url + 'commission_livreur/init_colis_facture/' + id_expediteur + '/' + type, 'colis-facture', [0, 6, 7], [0, 6, 7]);
        initDataTable_1('.table-historique-colis-factures', admin_url + 'commission_livreur/init_historique_colis_facture/' + facture_id + '/' + type, 'historique-colis-factures', [0, 6, 7], [0, 6, 7]);
    }
...
function init_facture() {
    var type = $('select[name="types"]').selectpicker('val');
    window.location.href = admin_url + 'commission_livreur/facture/false/' + type;
}
