$(document).ready(function() {
  ...
  $('body').on('click', '.choice-delete-modal', function(e) {
    e.preventDefault();
    var nbr = $('.delete-message-form input').length;
    if(nbr == 0){
  ...
  _validate_form($('.delete-message-form'), {}, manage_delete_message);
  ...
  $('body').on('click', '.show_more_messages', function() {
    ...
    var url = admin_url + 'live_chat/get_old_conversation_data_ajax/' + conversationid + '/' + type + '/' + receiverid;
    if($.isNumeric(page)){
      url += '/' + page;
    }
    // Hide Scroll
...
function init_list_contacts() {
  ...
  $('.list-contacts').load(admin_url + 'live_chat/get_list_contacts_data_ajax/' + input_search_contact);
}
//Reload Conversation
function init_conversation(type, id) {
  ...
  if(typeof(type) !== 'undefined' && typeof(id) !== 'undefined'){
    ...
    $('.bloc-conversation').load(admin_url + 'live_chat/get_conversation_data_ajax/' + type + '/' + id)
  }
}
...
function sync_conversation(type, id) {
  ...
  if(typeof(type) !== 'undefined' && typeof(id) !== 'undefined'){
    $.get(admin_url + 'live_chat/get_sync_conversation_data_ajax/' + type + '/' + id).success(function(response) {
...
function scroll_bottom_list_messages_conversation() {
  if($('.list-messages-conversation').html() != ''){
    $('.list-messages-conversation').scrollTop($('.list-messages-conversation')[0].scrollHeight);
    //$('.list-messages-conversation').animate({scrollTop: $('.list-messages-conversation')[0].scrollHeight}, "fast");
  }
}
//Add new message
function add_message() {
  ...
  if(!$.isNumeric(conversationid)){
    ...
    $.post(admin_url + 'live_chat/add_message', {
...
//Delete message
function manage_delete_message(form) {
    var conversationid = $('input[id="conversationid"]').val();
    var url  = form.action;
