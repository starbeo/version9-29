$(document).ready(function () {
    ...
    $('body').on('click', '.icon-return-mobile', function () {
        ...
        if (urlReferrer === '') {
            window.location.href = livreur_url + 'colis';
        } else {
            window.location.href = urlReferrer;
        }
    });
    //On click boutton add status
    $('body').on('click', '.btn-add-status', function () {
        if ($('#add-status').hasClass('display-none')) {
            $('button[id="btn-add-status"]').attr('disabled', false);
            $('#add-status').removeClass('display-none');
        } else {
            $('#add-status').addClass('display-none');
        }
    });
    //On click boutton close status
    $('body').on('click', '#btn-close-status', function () {
        if (!$('#add-status').hasClass('display-none')) {
            $('#add-status').addClass('display-none');
        }
        //Init
        $('#body-form-add-status select').selectpicker('val', '');
        $('#body-form-add-status input').val('');
    });
...
function manage_status(form) {
    $('button[id="btn-add-status"]').attr('disabled', true);
